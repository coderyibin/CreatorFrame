...
export default class Pomelo {
    ...
    addNetJuHua () : void {
        ...
        if (node) cc.find("Canvas").addChild(node);
    }
    removeJuHua () : void {
    ...
    initPomelo (account, password, cb : Function) : void {
        let self = this;
        pomelo.init({
            host : GameCtrl.getInstance().getGameConfig().ServerIp,
            port : GameCtrl.getInstance().getGameConfig().Port,
            log : true
        }, ()=> {
            var _name = name;
            var rou = ROUTE.GATE;

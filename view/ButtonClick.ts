...
export default class ButtonClick extends cc.Button {
    ...
    _bindClick (btn : cc.Button, name : string) : void {
        ...
        if (self._Comp[sName]) {
            ...
            self.addNodeEvent(sName, btn.node);
        } else {
     ...
     * 添加节点点击事件
     * @param name 点击事件的名称
     * @param node 点击事件绑定的按钮
     * @param data 自定义数据 选填
     */
    addNodeEvent (name : string, node : cc.Node, data ?: any) : void {
        ...
        node.on(cc.Node.EventType.TOUCH_END, (e)=>{   
            ...
            this._Comp[name](e);
            this._touch = false;
            this.scheduleOnce(()=>{this._touch = true}, 1);
        });

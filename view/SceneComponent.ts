...
export default class SceneComponent extends BaseComponent {
    ...
    _arrTime : Array<any>;
    _arrTimes : Array<any>;
    _arrReigsterUi : Array<any>;//弹窗或者其他动态生成的layer
    onLoad () : void {
        super.onLoad();
        this._arrEmit = ["runScene", "Msg"];
        this._arrReigsterUi = [MODULE.TIP_MSG];
        this._arrTime = [];
        this._arrTimes = [];
        this.initEvent();
        this.initValue();
    ...
    registerEvent () : void {
        let self = this;
        console.log('cur scene event:', self._arrEmit);
    ...
    Msg (data) : void {
        data = data || {};
        this.showLayer(MODULE.TIP_MSG, {content : data.content});
    }
